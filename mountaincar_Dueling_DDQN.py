...
model_t= torch.load("pth/MountainCar.pth")
...
<<<<<<< HEAD
        ...
        # total_reward += reward
        if done==True and j!=199:
            reward = 20
            print("get reward")
            next_state=None
        else:
            # reward = abs(observation_next[0] - observation[0])
            ...
            #         reward=torch.Tensor([1.0])
            # else:
            #     # if observation_next[0] < -0.5 and observation_next[0]>-0.55 :
            #     #     reward=torch.FloatTensor([abs(observation_next[1])])
            #     if observation_next[0]>0.3:
            #         reward=torch.FloatTensor([abs(observation_next[0])])
            #     else:
            #         reward=torch.FloatTensor([0.0])
            reward=-0.1
            reward+=abs(observation_next[1])
            reward+=abs(observation_next[0])/10
            next_state=torch.from_numpy(observation_next).type(torch.FloatTensor)
            next_state=torch.unsqueeze(next_state,0)
        total_reward+=reward
        Transition_mem.push(state,action,next_state,torch.FloatTensor([reward]))
            #
        replay()
            #
        state=next_state
