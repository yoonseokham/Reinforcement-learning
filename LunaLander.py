...
# model_m = torch.load("pth/LunaLanderWithRandom.pth")
...
for i in range(NUM_EPISODES):
    ...
    # if count==15:
        ...
        # by looking at their github repo,
        #  I can say that first 2 are position in x axis and y axis(hieght) ,
        #   other 2 are the x,y axis velocity terms,
    ...
    for j in range(MAX_STEPS):
            ...
            # print(observation_next)
            vectorShip=(-observation_next[0],-observation_next[1])
            temp1=math.sqrt(observation_next[0]**2+observation_next[1]**2)
            temp2=math.sqrt(observation_next[2]**2+observation_next[3]**2)
            speed_xist=math.sqrt(observation_next[2]**2+observation_next[3]**2)
            cosSimilarity=-100
            if temp1!=0 and temp2!=0:
                cosSimilarity=(vectorShip[0]*observation_next[2]+vectorShip[1]*observation_next[3])/(temp1*temp2)
            if done==True:
                if observation_next[6]==1 and observation_next[7]==1 and speed_xist<1 and abs(observation_next[0]) <0.06:
                    reward=300
                else :
                    reward=-100
            else:
                reward=0
                if cosSimilarity==-100:
                    if observation_next[6]==1 and observation_next[7]==1 and speed_xist<1 and abs(observation_next[0])<0.06:
                        reward=300
                    else :
                        reward=-100
                    ...
                    # else :
                    #     reward=-100
                    if cosSimilarity>0.5 and abs(observation_next[5])<0.2 and abs(observation_next[4])<0.2 and temp1<pretemp:
                        if temp1> 1:
                            reward=1+cosSimilarity
                        else:
                            if speed_xist<1:
                                reward=1+cosSimilarity
                            else :
                        ...
                        reward=-1
                    # reward-=2*temp1
                    # reward+=cosSimilarity
                    # reward-=speed_xist
                    # reward-=abs(observation_next[5])
                    # reward-=abs(observation_next[4])
                    # reward-=temp1
...
torch.save(model_m, "pth/LunaLanderWithRandom.pth")
for i in range(600):
        # env.render()
    with torch.no_grad():
        model_m.eval()
